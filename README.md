...
| Eval status | **17/17 locally** ([report](evals/reports/local.json)); the committed [production report](evals/reports/prod.json) is 15/15 from the last prod run and is regenerated by CI on the next deploy |
| Tests | 83 unit tests, no LLM or network required |
...
All numbers below are **measured** on the 17-question golden set against
...
Infrastructure adds a flat **$5.41/month** (Northflank `nf-compute-20`,
0.2 vCPU / 512 MB) regardless of volume, so it dominates the bill until
roughly **5,300 questions/month** on the golden-set mix (3,700 if every
question reaches the model). Below that, this is a fixed-cost service and
per-question spend is rounding error.
...
| Cut | To close |
|---|---|
| **Authentication** | The endpoint is public and unauthenticated. A per-IP rate limit *is* in place (SPEC §7.15) so a paid key cannot be trivially drained, but that is a spending bound, not access control. An API-key middleware plus the platform's WAF is roughly half a day including tests. |
| **Rate limiting beyond one instance** | The limiter is in-process, so several replicas would multiply the effective limit and a distributed source is not covered at all. Shared state (Redis) or limiting at the edge is what closes this; single-instance is the honest scope today. |
...
it: no authentication, no alerting on the logs it emits, single
...
| `tests/` | 83 unit tests, no network or LLM |
